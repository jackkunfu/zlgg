...
export default function(Vue){
    ...
    Vue.prototype._ajax = function(url, data, type){
        ...
        var isOtherUrl = url.substring(0, 5) === '/api/';
        return new Promise( (rs, rj)=>{
            $.ajax({
                type: type,
                url: isOtherUrl ? url : config.api + url,
                headers,
                dataType: 'json',
    ...
    Vue.prototype.ajax = async function(){
        try{
            var data = await this._ajax(...arguments);
            return data;
        }catch(e){
            console.log(arguments[0] + ' 错误:');
            console.log(e);
        }
    }
    ...
    Vue.prototype.getAllDist = async function(){
        ...
        if(res && res.code == 0 && res.data && res.data.length > 0) return res.data;
        else return [];
    }
